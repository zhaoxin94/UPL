...
from dassl.utils import (MetricMeter, AverageMeter, tolist_if_not,
                         count_num_param, load_checkpoint, save_checkpoint,
                         mkdir_if_missing, resume_from_checkpoint,
                         load_pretrained_weights)
from dassl.optim import build_optimizer, build_lr_scheduler
from dassl.data import DataManager

from clip666 import clip
from clip666.simple_tokenizer import SimpleTokenizer as _Tokenizer
...
from .utils import (select_top_k_similarity_per_class, caculate_noise_rate,
                    save_outputs, select_top_k_similarity, select_top_by_value,
                    caculate_noise_rate_analyze)
...
class UPLTrainer(TrainerX):
    ...
    def build_model(self):
        ...
        ) if cfg.TRAINER.UPLTrainer.PREC == "amp" else None
    ...
    def test(self, split=None, trainer_list=None):
        ...
        save_path = os.path.join(
            self.cfg.TEST.Analyze_Result_Path, self.cfg.DATASET.NAME,
            str(self.cfg.OPTIM.MAX_EPOCH) + '_' + str(self.cfg.SEED) + '_' +
            str(self.cfg.DATASET.NUM_SHOTS) + '_random_init' +
        ...
        while os.path.exists(
                os.path.join(
                    save_path,
                    'per_image_results_{}_{}.txt'.format(split, results_id))):
            results_id += 1
        self.per_image_txt_writer = open(
            os.path.join(
                save_path,
                'per_image_results_{}_{}.txt'.format(split, results_id)), 'w')
        self.per_class_txt_writer = open(
            os.path.join(
                save_path,
                'per_class_results_{}_{}.txt'.format(split, results_id)), 'w')
        ...
        for batch_idx, batch in enumerate(data_loader):
            ...
            if trainer_list is None or len(trainer_list) == 1:
                # 如果不是ensemble的测试
                output, image_features, text_features = self.model_inference(
                    input)
            ...
            self.evaluator.process(output, label, self.per_image_txt_writer,
                                   self.per_class_txt_writer)
        ...
        if split in ['all', 'train', 'test', 'novel', 'base']:
            if len(outputs_all) != 0:
                ...
                torch.save(
                    image_features_all,
                    os.path.join(save_path, '{}_v_features.pt'.format(split)))
                torch.save(
                    image_features_all,
                    os.path.join(save_path, '{}_targets.pt'.format(split)))
                torch.save(
                    outputs_all,
                    os.path.join(save_path, '{}_logits.pt'.format(split)))
                torch.save(
                    text_features_all,
                    os.path.join(save_path, '{}_l_features.pt'.format(split)))
    ...
    def zero_shot_analyze(self, trainer_list=None):
        ...
        for batch_idx, batch in tqdm(enumerate(data_loader)):
            ...
            if trainer_list is None or len(trainer_list) == 1:
                # 如果不是ensemble的测试
                output, image_features, text_features = self.model.zero_shot_forward(
                    input, self.device)
            else:
                # ensemble的测试
                outputs = [
                    t.model.zero_shot_forward(input, self.device)[0]
        ...
        predict_label_dict, _ = select_top_k_similarity_per_class(
            sstrain_outputs, sstrain_img_paths, -1, image_features, True)
        save_outputs(self.train_loader_x,
                     self,
                     predict_label_dict,
                     self.cfg.DATASET.NAME,
                     text_features,
                     backbone_name=self.cfg.MODEL.BACKBONE.NAME)
        caculate_noise_rate_analyze(predict_label_dict,
    ...
    def load_from_exist_file(self, file_path, model_names):
        ...
        for model in model_names:
            ...
            logist_path = os.path.join(
                model_path, '{}_logits.pt'.format(self.cfg.DATASET.NAME))
            if logits is None:
                logits = torch.load(logist_path)
            else:
                logits += torch.load(logist_path)

            info_path = os.path.join(model_path,
                                     '{}.json'.format(self.cfg.DATASET.NAME))
        ...
        predict_label_dict, predict_conf_dict = select_top_k_similarity_per_class(
            logits,
            sstrain_img_paths,
    ...
    def zero_shot_predict(self, trainer_list=None):
        ...
        if self.cfg.DATASET.CLASS_EQULE is True:
            if self.cfg.DATASET.CONF_THRESHOLD > 0:
                # 选择置信度大于一定值 & 选择
                predict_label_dict_1, predict_conf_dict_1 = select_top_k_similarity_per_class(
                    outputs, img_paths, K=self.cfg.DATASET.NUM_SHOTS)
                predict_label_dict_2, predict_conf_dict_2 = select_top_by_value(
                    outputs,
                    img_paths,
                ...
                predict_label_dict = dict(predict_label_dict_1,
                                          **predict_label_dict_2)
                predict_conf_dict = dict(predict_conf_dict_1,
                                         **predict_conf_dict_2)
                caculate_noise_rate(predict_label_dict,
                                    train_loader=self.train_loader_x,
                                    trainer=self)
            ...
            predict_label_dict, predict_conf_dict = select_top_k_similarity(
                outputs,
                img_paths,
                K=self.cfg.DATASET.NUM_SHOTS * text_features.shape[0])
            caculate_noise_rate(predict_label_dict,
                                train_loader=self.train_loader_x,
                                trainer=self)
            print('select {} samples'.format(len(predict_label_dict)))
    ...
    def test_with_existing_logits(self, logits, split='test'):
        ...
        self.evaluator.process(logits, label_all, self.per_image_txt_writer,
                               self.per_class_txt_writer)
        results = self.evaluator.evaluate()
        for k, v in results.items():
            tag = "{}/{}".format(split, k)
