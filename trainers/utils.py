...
def select_top_k_similarity_per_class(outputs,
                                      img_paths,
                                      K=1,
                                      image_features=None,
                                      is_softmax=True):
    ...
    for id in tqdm(list(set(ids.tolist()))):  # 标签去重
        ...
        if image_features is not None:
            ...
            if K >= 0:
                for img_path, img_feature, conf, logit in zip(
                        img_paths_class[:K], img_features[:K], conf_class[:K],
                        output_class):
                    if '/data/' in img_path:
                        img_path = './data/' + img_path.split('/data/')[1]
                    predict_label_dict[img_path] = [
                        id, img_feature, conf, logit
                    ]
            else:
                for img_path, img_feature, conf, logit in zip(
                        img_paths_class, img_features, conf_class,
                        output_class):
                    if '/data/' in img_path:
                        img_path = './data/' + img_path.split('/data/')[1]
                    predict_label_dict[img_path] = [
...
def select_top_k_similarity(outputs,
                            img_paths,
                            K=1,
                            image_features=None,
                            repeat=False):
    ...
    if image_features is not None:
        ...
        if K >= 0:
            ...
            for img_path, img_feature, conf, logit in zip(
                    img_paths_class, img_features, conf_class, output_class):
                predict_label_dict[img_path] = [id, img_feature, conf, logit]
    else:
        if K >= 0:
            for img_path, conf, id in zip(img_paths[:K], conf_class[:K],
                                          ids[:K]):
                predict_label_dict[img_path] = id
                predict_conf_dict[img_path] = conf
        else:
...
def select_top_by_value(outputs,
                        img_paths,
                        conf_threshold=0.95,
                        image_features=None,
                        repeat=False):
    ...
    if image_features is not None:
        img_features = image_features
        for img_path, img_feature, conf, logit in zip(img_paths_class,
                                                      img_features, conf_class,
                                                      output_class):
            if conf > conf_threshold:
                predict_label_dict[img_path] = [id, img_feature, conf, logit]
    else:
...
def caculate_noise_rate(predict_label_dict,
                        train_loader,
                        trainer,
                        sample_level=False):
    gt_label_dict = {}
...
def caculate_noise_rate_analyze(predict_label_dict,
                                train_loader,
                                trainer,
                                sample_level=False):
    gt_label_dict = {}
...
def save_outputs(train_loader,
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        for l, ip in zip(label, impath):
            ...
            if l not in gt_pred_label_dict:
                ...
                gt_pred_label_dict[l].append(
                    [ip, pred_label, pred_v_feature, conf, logits])
            else:
                pred_label = predict_label_dict[ip][0]
                pred_v_feature = predict_label_dict[ip][1]
                conf = predict_label_dict[ip][2]
    ...
    for label in gt_pred_label_dict:
        ...
        for item in gt_pred_label_dict[label]:
            impath, pred_label, pred_v_feature, conf, logits = item[0], item[
                1], item[2], item[3], item[4]
            v_features.append(pred_v_feature)
            logits_list.append(logits)
            v_dis = torch.dist(avg_feature, pred_v_feature, p=2)
            v_distance_dict_per_class[impath] = [
                idx, v_dis.item(), conf.item(), pred_label
            ]  # id, visual distance, confidence, predicted label
            idx += 1
    ...
    torch.save(
        v_features,
        './analyze_results/{}/{}_v_feature.pt'.format(backbone_name,
                                                      dataset_name))
    torch.save(
        text_features,
        './analyze_results/{}/{}_l_feature.pt'.format(backbone_name,
                                                      dataset_name))
    torch.save(
        logits_tensor,
        './analyze_results/{}/{}_logits.pt'.format(backbone_name,
                                                   dataset_name))

    with open(
            "./analyze_results/{}/{}.json".format(backbone_name, dataset_name),
            "w") as outfile:
        json.dump(v_distance_dict, outfile)


def select_top_k_similarity_per_class_with_high_conf(outputs,
                                                     img_paths,
                                                     K=1,
                                                     image_features=None,
                                                     repeat=False):
    ...
    selected_ids = sorted(class_avg_conf.items(),
                          key=lambda kv: (kv[1], kv[0]),
                          reverse=True)[:int(0.8 * len(class_avg_conf))]
    remain_ids = sorted(class_avg_conf.items(),
                        key=lambda kv: (kv[1], kv[0]),
                        reverse=True)[int(0.8 * len(class_avg_conf)):]
...
def select_top_k_similarity_per_class_with_low_conf(outputs,
                                                    img_paths,
                                                    conf_threshold,
                                                    remain_ids,
                                                    selected_ids,
...
def select_top_k_similarity_per_class_no_smaple(outputs,
                                                img_paths,
                                                no_sample_ids,
                                                K=16):
    # print(outputs.shape)
